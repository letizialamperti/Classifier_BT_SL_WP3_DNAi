...
from ORDNA.models.coral_loss_weighted_penalty import (
    WeightedCoralLossWithBoundaryPenalty,
)
...
class Classifier(pl.LightningModule):
    def __init__(
        ...
        pos_weights: torch.Tensor = None,          # shape: (num_classes-1,)
        importance_weights: torch.Tensor = None,   # shape: (num_classes-1,)
        lambda_boundary: float = 1.0,              # forza della penalty protetto vs non protetto
    ):
        super().__init__()
        ...
        # ---------------------------
        #   LOSS CORAL PESATA + PENALTY
        # ---------------------------
        # versione media per training/val logging
        self.loss_fn = WeightedCoralLossWithBoundaryPenalty(
            num_classes=num_classes,
            lambda_boundary=lambda_boundary,
            reduction='mean',
        )
        # versione "raw" (reduction=None) per avere loss per campione (istogramma)
        self.loss_fn_raw = WeightedCoralLossWithBoundaryPenalty(
            num_classes=num_classes,
            lambda_boundary=lambda_boundary,
            reduction=None,
        )
        ...
        #   BUFFER PER I PESI
        # ---------------------------
        # Li registriamo come buffer così vanno automaticamente sul device giusto
        ...
        # per logging a fine epoca di validazione
        self.validation_preds = []
        self.validation_labels = []
        self.validation_raw_loss = []  # per-sample loss CORAL+penalty
    ...
    def training_step(self, batch, batch_idx: int) -> torch.Tensor:
        ...
        combined_input = torch.cat((embeddings, habitats), dim=1)  # (B, input_dim)

        # Logits CORAL
        output = self(combined_input)  # (B, num_classes-1)

        # Livelli CORAL (0/1)
        levels = levels_from_labels(labels, self.num_classes)

        # Loss CORAL pesata + boundary penalty
        loss = self.loss_fn(
            output,
            levels,
            labels,
        ...
        # Predizioni discrete per metriche
    ...
    def validation_step(self, batch, batch_idx: int):
        ...
        # Loss per-campione (per istogramma)
    ...
    def on_validation_epoch_end(self):
        ...
        axes[2].set_xlabel('Per-sample CORAL+penalty loss')
