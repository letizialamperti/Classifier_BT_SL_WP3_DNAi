...
class CoralHead(nn.Module):
    """
    CORAL last layer (authors-style):
        logits_k = w^T x + b_k
    with shared weights w and per-threshold biases b_k.
    """
    def __init__(self, in_features: int, num_classes: int):
        super().__init__()
        self.fc = nn.Linear(in_features, 1, bias=False)          # shared weights
        self.bias = nn.Parameter(torch.zeros(num_classes - 1))   # per-threshold bias

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return self.fc(x) + self.bias  # broadcast: (B,1)+(K-1,) -> (B,K-1)
...
class Classifier(pl.LightningModule):
    def __init__(
        ...
        # Backbone (same as before, without the final Linear to K-1)
        ...
        # CORAL head with shared weights (authors-style)
        self.coral_head = CoralHead(in_features=256, num_classes=num_classes)
