...
# CORAL: from ORDNA.models.classifier import coral_to_label
from ORDNA.models.dann_classifier_mae import DANNClassifier
...
def calculate_class_weights_from_csv_coral(protection_file: Path, num_classes: int) -> torch.Tensor:
    ...
    NOTA: questa funzione ora è inutile per la MAE, ma viene lasciata per compatibilità
    se il costruttore del modello accetta ancora class_weights.
    """
...
def main():
    ...
    for split_file in split_files:
        ...
        # Pesi CORAL (non più usati per la MAE, ma calcolati se il modello li accetta)
        ...
        model = DANNClassifier(
            ...
            class_weights=class_weights  # ignorati se non usati nel modello
        )
        ...
        with torch.no_grad():
            for emb, lab, dom in val_loader:   # il dataset restituisce (emb, label, dom)
                ...
                # ---- Task: output continuo dal modello MAE ----
                pred_cont = model(x)  # [B] o [B,1] a seconda di come hai implementato forward
                if pred_cont.dim() == 2 and pred_cont.size(1) == 1:
                    pred_cont = pred_cont.squeeze(1)

                # converto in classi discrete: round + clamp
                pred_cls = torch.round(pred_cont).long()
                pred_cls = torch.clamp(pred_cls, 0, args.num_classes - 1)

                pred_np = pred_cls.detach().cpu().numpy()
                ...
                preds_list.extend(pred_np.tolist())
